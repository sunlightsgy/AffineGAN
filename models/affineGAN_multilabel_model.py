...
class AffineGANMultilabelModel(BaseModel):
    def name(self):
        return "AffineGANMultilabelModel"
    ...
    def get_alpha(self, f_t0, f_t, f_t0_res):
        if self.opt.cal_a == "norm":
            return torch.abs(torch.sum((f_t - f_t0) * f_t0_res)) / (
                f_t0_res.norm() + 1e-6
            )
        elif self.opt.cal_a == "no_norm":
            return torch.abs(torch.sum((f_t - f_t0) * f_t0_res))
        else:
            return None
    ...
    def forward(self):
        ...
        self.t0_reconstruct, f_t0 = self.netG(self.input_A, 1.0, 0.0, self.label)
        ...
        _, f_t0_res = self.netG(self.input_A, 0.0, 1.0, self.label)
        ...
        for img_idx in range(self.opt.train_imagenum):
            ...
            t_reconstruct, f_t = self.netG(real_B, 1.0, 0.0, self.label)
            self.B_reconstruct_img_list.append(t_reconstruct[:, :3, :, :])
            f_t = torch.squeeze(f_t)
            alpha = self.get_alpha(f_t0, f_t, f_t0_res)
            alpha_list_torch.append(alpha.view(1))

            fake_B, _ = self.netG(self.input_A, 1.0, float(alpha), self.label)

            self.real_B_list.append(real_B)
            self.fake_B_list.append(fake_B[:, :3, :, :])

            if not self.opt.no_patch:
                real_B_patch = self.input_B_patch_list[img_idx]
                real_B_img_patch = real_B * real_B_patch
                fake_B_img_patch = fake_B[:, :3, :, :] * real_B_patch
    ...
    def backward_D(self):
        ...
        for img_idx in range(self.opt.train_imagenum):
            ...
            pred_fake, fake_cls = self.netD(fake_AB.detach())
            self.loss_D_fake = self.criterionGAN(pred_fake, False)

            # Real
            real_AB = torch.cat((self.input_A, self.real_B_list[img_idx]), 1)
            pred_real, real_cls = self.netD(real_AB.detach())
            self.loss_D_real = self.criterionGAN(pred_real, True)
            self.loss_D_cls = F.binary_cross_entropy_with_logits(
                real_cls, self.label, size_average=False
            ) / real_cls.size(0)

            # Combined loss
            loss_D += (
                self.loss_D_fake + self.loss_D_real
            ) * 0.5 + self.loss_D_cls * 0.5
    ...
    def backward_D_patch(self):
        ...
        for img_idx in range(self.opt.train_imagenum):
            ...
            pred_fake_patch, fake_cls = self.netD_Patch(fake_AB_patch.detach())
            ...
            pred_real_patch, real_cls = self.netD_Patch(real_AB_patch.detach())
            self.loss_D_real_patch = self.criterionGAN(pred_real_patch, True)
            self.loss_D_patch_cls = F.binary_cross_entropy_with_logits(
                real_cls, self.label, size_average=False
            ) / real_cls.size(0)
            # Combined loss
            loss_D_patch += (
                self.loss_D_fake_patch + self.loss_D_real_patch
            ) * 0.5 + self.loss_D_patch_cls * 0.5
    ...
    def backward_G(self):
        ...
        for img_idx in range(self.opt.train_imagenum):
            fake_AB = torch.cat((self.input_A, self.fake_B_list[img_idx]), 1)
            pred_fake, fake_cls = self.netD(fake_AB)
            current_loss_G_GAN_D1 = self.criterionGAN(pred_fake, True)
            current_loss_G_D1_cls = F.binary_cross_entropy_with_logits(
                fake_cls, self.label, size_average=False
            ) / fake_cls.size(0)
            loss_G_cls += current_loss_G_D1_cls
            ...
            if not self.opt.no_patch:
                ...
                pred_fake_patch, fake_patch_cls = self.netD_Patch(fake_AB_patch)
                ...
                current_loss_G_patch_cls = F.binary_cross_entropy_with_logits(
                    fake_patch_cls, self.label, size_average=False
                ) / fake_patch_cls.size(0)
                loss_G_patch_cls += current_loss_G_patch_cls / (
                    self.opt.train_imagenum + 0.0
                )
            ...
            loss_G += (
                current_loss_G_GAN_D1
                + current_loss_G_L1
                + current_img_recons_loss
                + current_loss_G_D1_cls
            )
        ...
        self.loss_G_cls = loss_G_cls
        self.loss_G_patch_cls = loss_G_patch_cls
    ...
    def test(self):
        with torch.no_grad():
            self.label = torch.FloatTensor([[0, 0, 0.5, 0, 0.5, 0, 0]]).to(self.device)
            # print(self.label)
            self.fake_B_list = []
            for i in range(int(1.0 / self.opt.interval)):
                self.fake_B_list.append(
                    self.netG(self.input_A, 1.0, self.opt.interval * i, self.label)[0][
                        :, :3, :, :
                    ]
                )
